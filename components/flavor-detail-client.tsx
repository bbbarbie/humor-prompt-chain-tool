...
export function FlavorDetailClient({
  ...
  async function deleteFlavor() {
    if (!window.confirm("Delete this flavor? This is blocked until all linked steps are removed.")) {
      return;
    }
    ...
    const json = (await response.json().catch(() => null)) as { error?: string } | null;
    if (!response.ok) {
      setErrorMessage(json?.error || "Unable to delete flavor.");
      return;
    }
  ...
  async function runFlavor() {
    if (!selectedImageId) {
      setErrorMessage("Select a test image first.");
    ...
    try {
      ...
      setCurrentResult(null);
      setErrorMessage(error instanceof Error ? error.message : "Unable to generate captions.");
    } finally {
      setRunnerLoading(false);
    }
  }

  function renderSelect(label: string, value: string, options: SelectOption[], onChange: (value: string) => void) {
    return (
      ...
      <div className="space-y-5">
        <label className="block space-y-2">
          ...
          <textarea
            ...
            onChange={(event) => setStepDraft({ ...stepDraft, llm_system_prompt: event.target.value })}
        ...
        <label className="block space-y-2">
          ...
          <textarea
            ...
            onChange={(event) => setStepDraft({ ...stepDraft, llm_user_prompt: event.target.value })}
            className="w-full rounded-3xl border bg-transparent px-4 py-4 outline-none transition focus:border-[color:var(--border-strong)]"
          />
        </label>
        ...
        <div className="grid gap-4 md:grid-cols-2">
          <label className="block space-y-2">
            ...
            <input
              ...
              onChange={(event) => setStepDraft({ ...stepDraft, llm_temperature: event.target.value })}
              className="w-full rounded-2xl border bg-transparent px-4 py-3 text-sm outline-none transition focus:border-[color:var(--border-strong)]"
            />
          </label>
        ...
        <details className="rounded-3xl border border-[color:var(--border)] bg-[color:var(--panel-muted)] p-4">
          ...
          <div className="mt-4 grid gap-4 md:grid-cols-2">
            {renderSelect("Model", stepDraft.llm_model_id, modelOptions, (value) => setStepDraft({ ...stepDraft, llm_model_id: value }))}
            {renderSelect("Input type", stepDraft.llm_input_type_id, inputTypeOptions, (value) =>
              setStepDraft({ ...stepDraft, llm_input_type_id: value }),
            )}
            {renderSelect("Output type", stepDraft.llm_output_type_id, outputTypeOptions, (value) =>
              setStepDraft({ ...stepDraft, llm_output_type_id: value }),
            )}
            {renderSelect("Step type", stepDraft.humor_flavor_step_type_id, stepTypeOptions, (value) =>
              setStepDraft({ ...stepDraft, humor_flavor_step_type_id: value }),
            )}
          </div>
        </details>
      </div>
  ...
  return (
    <>
      ...
      <section className="grid gap-6 xl:grid-cols-[1.3fr_0.9fr]">
        <div className="rounded-[32px] border border-[color:var(--border)] bg-[color:var(--panel-strong)] p-7 shadow-[var(--shadow-soft)]">
          <div className="flex flex-wrap items-start justify-between gap-4">
            <div>
              <p className="text-[11px] uppercase tracking-[0.32em] text-[color:var(--muted-foreground)]">Flavor detail</p>
              <h2 className="mt-3 font-serif text-6xl leading-none tracking-tight">{flavor.description || "Untitled flavor"}</h2>
        ...
        <aside className="rounded-[32px] border border-[color:var(--border)] bg-[color:var(--panel)] p-7 shadow-[var(--shadow-soft)]">
          <p className="text-[11px] uppercase tracking-[0.32em] text-[color:var(--muted-foreground)]">Testing panel</p>
          <h3 className="mt-3 font-serif text-4xl tracking-tight">Run this flavor</h3>
          ...
          {selectedImage ? (
            <div className="mt-5 overflow-hidden rounded-[28px] border border-[color:var(--border)] bg-[color:var(--panel-strong)]">
              <div className="grid gap-0 lg:grid-cols-[1.1fr_0.9fr]">
                <div className="relative min-h-72 bg-[color:var(--panel-muted)]">
                  {selectedImage.url ? (
                    <Image
          ...
          >
            {runnerLoading ? "Generating..." : "Generate captions"}
          </button>
        </aside>
      </section>
